...
ssh "${SSH_OPTS[@]}" "$SSH_TARGET" \
  "sudo python3 $REMOTE_SCRIPT --bundle $REMOTE_BUNDLE --root '$REMOTE_ROOT' --bundle-id '$bundle_id' --container '$CONTAINER' --schema $REMOTE_SCHEMA"
ssh "${SSH_OPTS[@]}" "$SSH_TARGET" "rm -f '$REMOTE_SCRIPT' '$REMOTE_BUNDLE' '$REMOTE_SCHEMA'"
