...
```bash
PYTHONPATH=src uv run python -c 'from pathlib import Path; from jurisprudence_case_schema import write_case_json_schema; write_case_json_schema(Path("schemas/residenciafiscal-case-v3.schema.json"))'
```
