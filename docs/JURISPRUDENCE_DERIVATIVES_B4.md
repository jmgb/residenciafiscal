...
```bash
uv run python -c 'from pathlib import Path; from jurisprudence_case_retrieval_schema import write_retrieval_json_schema; write_retrieval_json_schema(Path("schemas/residenciafiscal-retrieval-v1.schema.json"))'
```
